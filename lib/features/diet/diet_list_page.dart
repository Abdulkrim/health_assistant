...
class DietListPage extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Diet>>(
          ...
          builder: (context, snapshot) {
            ...
            if (diets.isEmpty) return Center(child: Text('لايوجد حميات مضافة'));
            return ListView.separated(
              itemCount: diets.length,
              padding: EdgeInsets.only(
                  bottom: 10.0.h, left: 1.0.w, right: 1.0.w, top: 1.0.h),
              ...
              child: Column(
                ...
                children: [
                  ...
                  showWrapContent(
                      ...
                      isMedicines: true),
                  showWrapContent(
                      title: 'الاطعمة المسموحة',
                      dietId: diet.id,
                      isAllowed: true,
                      isMedicines: false),
                  showWrapContent(
                      title: 'الاطعمة الممنوعة',
                      dietId: diet.id,
                      isAllowed: false,
                      isMedicines: false),
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(
                          child: Text(
                            "${DateFormat('yyyy-MM-dd').format(diet.startDate)}  -->  ${DateFormat('yyyy-MM-dd').format(diet.endDate)}",
  ...
  showWrapContent(
      {String title, bool isAllowed, int dietId, bool isMedicines}) {
    return FutureBuilder(
      future: isMedicines
          ? db.getDietMedicines(dietId, isAllowed)
          : db.getDietFoods(dietId, isAllowed),
      builder: (context, snapshot) {
