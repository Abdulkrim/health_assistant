...
class _AddAnalysisPageState extends State<AddAnalysisPage> {
          ...
          children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Expanded(
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Center(
                                child: Text(
                                  analysisName?.name ?? "اضغط لإختيار اسم",
                        ...
                        if (checkValidate &&
                            (analysisName?.name?.isEmpty ?? true))
                          Padding(
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                padding:
                    EdgeInsets.symmetric(vertical: 1.0.h, horizontal: 4.0.w),
                child: Row(
                  children: [
                    Expanded(
                      child: Center(
                ...
                child: Center(
                  child: Row(
                    children: [
                      Expanded(
                          child: Text(
                              '   تاريخ ${widget.name == null ? "التحليل" : "النتيجة"}')),
                      Expanded(
                          child: Text(
            ...
            ElevatedButton(
                ...
                onPressed: () async {
                  if (analysisName?.name?.isEmpty ?? true) {
                    checkValidate = true;
                    setState(() {});
                    ...
                    (await db.getAllAnalysis(analysis.nameId))
                        .forEach((element) {
                      if (element.lastDate.isAfter(lastDate) ?? false)
                        lastDate = element.lastDate;
                    });
                    db.updateAnalysisName(
                        analysisName.copyWith(lastDate: lastDate));
                    Navigator.pop(context);
                  }
                },
  ...
  Future<AnalysisName> getName() async {
    ...
    return showModalBottomSheet<AnalysisName>(
        context: context,
                      ...
                      CustomTextField(
                        ...
                        onIconClicked: () async {
                          if (nameController.text.trim().isNotEmpty) {
                            if (!list.any((element) =>
                                element.name == nameController.text.trim())) {
                              await db.addAnalysisName(
                                  AnalysisName(name: nameController.text));
