    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () => Navigator.pushNamed(
            context, NameScreen.add_analysis_page,
            arguments: name),
      ),
      body: StreamBuilder<List<Analysis>>(
          stream: doctor == null
              ? db.watchAllAnalysis(name.id)
              : db.watchAllDoctorAnalysis(name.id, doctor.id),
              ...
              itemBuilder: (context, i) {
                return GestureDetector(
                  onTap: () => Navigator.pushNamed(
                      context, NameScreen.analysis_details_page,
                      arguments: analysis[i]),
                  child: Card(
                    elevation: 4,
                            ...
                            Expanded(
                                ...
                                child: Align(
                                  alignment: Alignment(0.0,
                                      analysis[i].value == null ? 0.5 : 2.0),
                                  child: Text(
                                    analysis[i].value == null
                                        ? "اضغط للتفاصيل"
                                        : analysis[i].value.toString(),
                                    style: TextStyle(
                                        color: Theme.of(context).primaryColor,
                                        fontSize: analysis[i].value == null
                                            ? 14.0.sp
                                            : 60.0.sp,
                            ...
                            onPressed: () {
                              showDialog(
                                  ...
                                  builder: (cxt) => AlertDeleteDialog(
                                        onDelete: () async {
                                          await db
                                              .deleteAnalysis(analysis[i].id);
                                          if ((await db.getAllAnalysis(
                                                  analysis[i].nameId))
                                              .isEmpty) {
                                            await db.deleteAnalysisName(
                                                analysis[i].nameId);
